...
class Program
{
    static void Main()
    {
        ...
        if (startIndex >= 0)
        {
            ...
            {
                var oldBlock = content.Substring(startIndex, endIndex - startIndex);
                var newBlock = @"foreach (var c in report.CausasRaiz.Take(8))
            {
                ...
                File.WriteAllText(@"C:\Users\Jorge Ramos\Downloads\TDM-ORIGINAL-ZIP\TDM-v1.0-rc18.21.0-FIX93-CLEAN-R8-P1-P2-COMPILEFIX\src\TDM.Reporting\ReportExporter.cs", newContent, Encoding.UTF8);
