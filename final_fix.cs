...
class Program
{
    static void Main()
    {
        ...
        content = Regex.Replace(content,
            @"public static async Task<ReportExportResult> ExportAsync\(\s+DiagnosticReport report,\s+string directory,\s+CancellationToken ct = default\)",
            "public static async Task<ReportExportResult> ExportAsync(\r\n        DiagnosticReport report,\r\n        string directory,\r\n        CancellationToken ct = default,\r\n        DiagnosticReport? previousReport = null)");

        // 2. Fix the file write section with diff injection
