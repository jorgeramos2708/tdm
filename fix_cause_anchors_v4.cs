...
// Fix the causes loop to use position-based anchor (cau-1, cau-2, etc.)
var oldCauses = @"foreach (var c in report.CausasRaiz.Take(8))
            {
                var causeAnchor = ""cau-"" + c.Id.Replace("":"", ""-"").Replace(""/"", ""-"");
...
var fileContent = File.ReadAllText(path, Encoding.UTF8);
fileContent = fileContent.Replace(oldCauses, newCauses);
